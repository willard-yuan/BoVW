...
// Function: this function transform sift to rootsift
// Input: STL vector
// Output: STL vector
// Note: rootSIFT = sqrt(sift/sum(sift)), after do square-root, this is no need to do L2 normalization again, because the sum of each square-root element's square is 1.
std::vector<float> siftDesctor::rootsift(std::vector<float> &dst){
    std::vector<float> rootSIFT(dst.size());
    // Do L1 normalization
    float sum_dst = std::accumulate(dst.begin(), dst.end(), 0.0); //float sum_dst = 1.0;
    std::transform(dst.begin(), dst.end(), rootSIFT.begin(), bind2nd(std::divides<float>(), sum_dst));
    // Do square-root
    std::transform(rootSIFT.begin(), rootSIFT.end(), rootSIFT.begin(), (float(*)(float)) sqrtf);
    return rootSIFT;
}
